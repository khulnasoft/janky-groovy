		...
		stage('Checkout ' + repo) {
			checkout(
				...
				scm: [
					$class: 'GitSCM',
					userRemoteConfigs: [[
						url: 'https://github.com/khulnasoft/' + repo + '.git',
					]],
					branches: [[name: '*/master']],
					extensions: [
						[
		...
		withEnv([ 'repo=' + repo ]) {
			stage('Generate ' + repo) {
				repoChanges = sh(returnStdout: true, script: '''#!/usr/bin/env bash
					...
					url="https://github.com/khulnasoft/$repo"
				...
				sh '''#!/usr/bin/env bash
					...
					if [ "$timestamp" -gt 0 ] && [ -n "${changes:-}" ]; then
						...
						if git -C oi add library/ && git -C oi commit "${commitArgs[@]}"; then
							git -C oi push -f git@github.com:khulnasoft-bot/docker-images.git "HEAD:refs/heads/elk"
							pushed=1
						fi
					fi

					if [ -z "$pushed" ]; then
						git -C oi push --delete git@github.com:khulnasoft-bot/docker-images.git refs/heads/elk
					fi
				'''
			}
		}
