...
node {
	...
	checkout(
		scm: [
			$class: 'GitSCM',
			userRemoteConfigs: [[
				url: 'https://github.com/khulnasoft/docs.git',
			]],
			branches: [[name: '*/master']],
			extensions: [
				[
	...
	for (repo in repos) { withEnv(['repo=' + repo]) {
		badNews = sh(returnStdout: true, script: '''#!/usr/bin/env bash
			...
			if [ "$d" -gt "$OUTDATED_CUTOFF" ] || [ "$count" -eq 0 ]; then
				...
				echo "- https://github.com/khulnasoft/docker-images/commit/$commit"
				echo "- https://github.com/khulnasoft/docker-images/pulls?q=label%3Alibrary%2F$repo"
				...
				if [ -d "docs/$repo" ] && docsCommit="$(git -C docs log -1 --format=format:%H -- "$repo/" ":(exclude)$repo/README.md")" && [ -n "$docsCommit" ]; then
					...
					echo 'docs:'
					echo "- https://github.com/khulnasoft/docs/commit/$docsCommit ($docsD)"
					echo "- https://github.com/khulnasoft/docs/tree/$docsCommit/$repo#readme"
					if [ -f "docs/$repo/deprecated.md" ]; then
						echo "- https://github.com/khulnasoft/docs/blob/$docsCommit/$repo/deprecated.md"
					fi
				fi
