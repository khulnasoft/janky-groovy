...
def vars = fileLoader.fromGit(
	...
	'https://github.com/khulnasoft/janky-groovy.git', // repo
	'master', // branch
	null, // credentialsId
	'', // node/label
)
...
node {
	...
	stage('Checkout') {
		checkout(
			...
			scm: [
				$class: 'GitSCM',
				userRemoteConfigs: [[
					url: 'https://github.com/khulnasoft/docker-images.git',
				]],
	...
	def dsl = '''
		...
		// https://github.com/khulnasoft/meta/blob/subset/subset.txt
		def subset = []
		new URL('https://raw.githubusercontent.com/khulnasoft/meta/subset/subset.txt').eachLine { line ->
			subset << line
		}
		if (subset.size == 0) {
			...
			for (arch in arches) {
				...
				for (img in archImages[arch]) {
					...
					if (img == '_trigger') {
						...
						desc = 'Trigger all the pinned things! (Jenkins SCM triggering does not work well for our use case); https://github.com/khulnasoft/docker-images/tree/master/.external-pins'
						...
						desc = """
							...
							<ul>
								...
								<li><a href="https://github.com/khulnasoft/docker-images/blob/master/library/${img}"><code>official-images/library/${img}</code></a></li>
							</ul>
						"""
						...
						definition {
							cpsScm {
								scm {
									git {
										remote {
											url('https://github.com/khulnasoft/janky-groovy.git')
										}
										branch('*/master')
										extensions {
											cleanAfterCheckout()
