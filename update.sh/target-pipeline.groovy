...
node {
	...
	stage('Checkout') {
		...
		sh '''#!/usr/bin/env bash
			...
			git -C oi config user.email 'doi+khulnasoft-bot@docker.com'
			git -C repo config user.name 'Docker Library Bot'
			git -C repo config user.email 'doi+khulnasoft-bot@docker.com'

			# https://github.com/moby/moby/issues/30973 🤦
			# docker build --pull --tag oisupport/update.sh 'https://github.com/khulnasoft/janky-groovy.git#:update.sh'
			tempDir="$(mktemp -d -t docker-build-janky-XXXXXXXXXX)"
			trap 'rm -rf "$tempDir"' EXIT
			git clone --depth 1 https://github.com/khulnasoft/janky-groovy.git "$tempDir"
