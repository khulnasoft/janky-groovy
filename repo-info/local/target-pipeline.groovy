	...
	stage('Checkout') {
		checkout(
			...
			scm: [
				$class: 'GitSCM',
				userRemoteConfigs: [[
					url: 'git@github.com:khulnasoft/repo-info.git',
	...
	stage('Gather') {
		def tagGroups = sh(
			...
			""" + '''
				...
				case "$repo" in
					neo4j) # https://github.com/khulnasoft/docker-images/issues/1864 (tag sprawl of ancient, unsupported versions)
	...
	ansiColor('xterm') { dir('ri') {
		...
		stage('Commit') {
			sh """
				...
				git config user.email 'doi+khulnasoft-bot@docker.com'
